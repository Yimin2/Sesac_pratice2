...
async function getReveiwRating(id) {
    const data = await axios.get(`${BASE_URL}/products/${id}`)
    const res = data.data.reviews.map((reviews) => {
...
async function getPostsTagList() {
    const data = await axios.get(`${BASE_URL}/posts/tag-list`)
...
async function getPostsTag(search) {
    const data = await axios.get(`${BASE_URL}/posts/tag/${search}`)
    console.log(data.data)
}
...
async function getSearchProductsTotal(id){
    const data = await axios.get(`${BASE_URL}/products/search?q=${id}`)
...
async function main() {
    ...
    await getReveiwRating(10)
    console.log(border)

    await getPostsTagList()
    console.log(border)

    await getPostsTag("life")
    console.log(border)

    await getSearchProductsTotal("phone")
}
