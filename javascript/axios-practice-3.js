...
async function getRecipe(type) {
    const data = await axios.get(`${BASE_URL}/recipes/meal-type/${type}`)
...
async function userSerach(username) {
    const data = await axios.get(`${BASE_URL}/users/search?q=${username}`)
    console.log(data.data)
}
...
async function todoSearch(id) {
    const data = await axios.get(`${BASE_URL}/todos/user/${id}`)
    console.log(data.data)
}
...
async function getAllComment() {
    ...
    const res = data.data.comments.map((comment) => {
        return {
            body: comment.body,
            likes: comment.likes,
...
async function main() {
    // await getRecipe("lunch")
    // await addRecipe()
    // await userSerach("anna")
    // await todoSearch(9)
    await getAllComment()
}
