...
animals.forEach((str) => (console.log(str)))
...
// 출력 결과는 배열 형식으로 출력한다
fruits.forEach((element) => (console.log(`맛있는 ${element}`)))
...
const newArray1 = numbers2.map((num) => num * num)
...
const newArray2 = names.map((str) => `${str}님`)
...
const newArray3 = numbers3.filter((num) => num % 2 === 0 ? true : false)
...
const newArray4 = people.filter(({age, name}) => {
...
const newArray5 = numbers4.reduce((acc, num2) => acc + num2, 0)
...
const newArray6 = people1.find(({id, name}) => {
...
const newArray7 = products
    .filter(({category}) => category === "전자제품")
